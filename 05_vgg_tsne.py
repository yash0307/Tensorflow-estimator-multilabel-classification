...
CLASS_COLORS = {
    ...
    'chair':(0.4, 0.1, 0.8),
    'cow':(0.5, 0.1, 0.7),
    'diningtable':(0.8, 0.6, 0.7),
    'dog':(0.2, 0.7, 0.8),
    'horse':(0.8, 0.1, 0.9),
    'motorbike':(0.9, 0.45, 0.35),
    'person':(0.45, 0.95, 0.25),
    'pottedplant':(0.75, 0.65, 0.35),
    'sheep':(0.15, 0.85, 0.95),
    'sofa':(0.2, 0.5, 0.3),
    'train':(0.3, 0.5, 0.2),
    'tvmonitor':(0.65, 0.35, 0.55),
}
...
def main():
    ...
    random_picks = np.random.randint(low=0, high=len(im_names), size=1000)
    XX = np.zeros((1000, 4096), dtype='float')
    YY = np.zeros((1000, 20), dtype='float')
    for i in range(0, len(random_picks)):
        im_num = random_picks[i]
        im_name = im_names[im_num]
        im_rep = rep_dict[im_name]
        y_rep = eval_labels[im_num,:]
        XX[i,:] = im_rep
        YY[i,:] = y_rep
    tsne = TSNE(n_components=2, perplexity=30.0, verbose=1, n_iter=1000)
    tsne_results = tsne.fit_transform(XX)
    print(tsne_results.shape)
    #c_mean = np.zeros((tsne_results.shape[0],1), dtype='float')
    fig = plt.figure()
    ax = fig.add_subplot(111)
    for i,j in enumerate(tsne_results):
        classes = YY[i,:]
        pos_idxs = np.where(classes > 0)[0]
        c = (0,0,0)
        for g_idx in pos_idxs:
            g_c = CLASS_COLORS[CLASS_NAMES[g_idx]]
            c = [sum(x) for x in zip(c,g_c)]
        c = [float(cc)/float(len(pos_idxs)) for cc in c]
        ax.scatter(j[0], j[1], color=c)
    list_patches = []
